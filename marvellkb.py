...
if authenticate_user():
    with st.sidebar:
      image = Image.open("assets/jadegloballogo.png")
      image = st.image('assets/jadegloballogo.png',width=260)
      image = Image.open("assets/Knowledgelogo.png")
      image = st.image('assets/Knowledgelogo.png',width=265)
   
    image = Image.open("assets/KB_Top1.jpg")
    new_image = image.resize((830, 100))
    st.image(new_image) 
    
    query = st.chat_input("Enter your question:")
    st.markdown("""
    ##### Empower users to derive insights from the knowledge base documents and collaterals stored in SharePoint and other knowledge repositories.
